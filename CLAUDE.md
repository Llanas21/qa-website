...
- **`assets/img/querify-logo.svg`** — the brand mark, referenced via `<img class="glyph">` (not inlined) in every page's header/footer `.brand` block, and reused as the favicon (`<link rel="icon" type="image/svg+xml">`). Swap this one file to change the logo everywhere; don't reintroduce per-page inline `<svg>` copies.
