...
- **`templates.js`** — the message templates (bienvenida/recordatorio/valor/cierre/pago) as both plain text (for email/logs) and Meta template params (for WhatsApp). `fill()` supports any `{{N}}` placeholder (not just 1-3) so the `pago` template can carry monto + liga de pago.
